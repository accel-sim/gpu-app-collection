...
def run_options(progname, cmd, args):
    usage_string = progname + " run BENCHMARK VERSION INPUT [PLATFORM=default]\nRun version VERSION of BENCHMARK with platform PLATFORM and data set INPUT"
    parser = OptionParser(usage=usage_string)
...
def debug_options(progname, cmd, args):
    usage_string = progname + " debug BENCHMARK VERSION INPUT\nRun version VERSION of BENCHMARK with data set INPUT in debugger"
    parser = OptionParser(usage=usage_string)
    parser.add_option('-C', "--no-check",
                      action="store_false", dest="check", default=True,
                      help="Skip the output check for this benchmark")
