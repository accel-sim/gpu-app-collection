...
__global__ void shared_bw(uint32_t *startClk, uint32_t *stopClk, uint32_t *dsink, uint32_t *l1, uint32_t stride){
...
int main(){
	...
	gpuErrchk( cudaMemcpy(posArray_g, posArray, TOTAL_THREADS*sizeof(uint32_t), cudaMemcpyHostToDevice) );

	shared_bw<<<BLOCKS_NUM,THREADS_PER_BLOCK>>>(startClk_g, stopClk_g, dsink_g, posArray_g, stride);
    ...
    double bw;
	bw = (double)(ITERS*TOTAL_THREADS*4*2)/((double)(stopClk[0]-startClk[0]));
