...
__global__ void wgmma_latency_kernel(uint32_t *startClk, uint32_t *stopClk, uint32_t *checksum) {
  ...
  auto sA_layout = tile_to_shape(GMMA::Layout_K_INTER_Atom<ElementA>{},
                                 make_shape(shape<0>(TileShape_MNK{}), shape<2>(TileShape_MNK{}), Int<PIPE>{}));
  auto sB_layout = tile_to_shape(GMMA::Layout_K_INTER_Atom<ElementB>{},
                                 make_shape(shape<1>(TileShape_MNK{}), shape<2>(TileShape_MNK{}), Int<PIPE>{}));

  // Allocate shared memory with proper size
  __shared__ ElementA smem_A[cosize_v<decltype(sA_layout)>];
  __shared__ ElementB smem_B[cosize_v<decltype(sB_layout)>];
  ...
  for (int j = 0; j < REPEAT_TIMES; j++) {
    // Call the fma method
    cute::gemm(tiled_mma, tCrA(_,_,0,0), tCrB(_,_,0,0), accum);
  }
