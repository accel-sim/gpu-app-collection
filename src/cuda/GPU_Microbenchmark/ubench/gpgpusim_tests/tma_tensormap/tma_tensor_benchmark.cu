...
__global__ void test_kernel(const CUtensorMap *tensor_map, int *mat, TestType test_type, int width_stride, int run_iters);
__device__ void test_UTMAPF_kernel(const CUtensorMap *tensor_map, int x, int y, int run_iters);
__device__ void test_UTMALDG_kernel(const CUtensorMap *tensor_map, int x, int y, int run_iters);
__device__ void test_UTMASTG_kernel(const CUtensorMap *tensor_map, int x, int y, int run_iters);
__device__ void test_UTMAREDG_kernel(const CUtensorMap *tensor_map, int x, int y, int run_iters);
...
int main(int argc, char *argv[]) {
    ...
    cudaMalloc(&d_tensor_map, sizeof(CUtensorMap));
    ...
    if (res != CUDA_SUCCESS) {
        fprintf(stderr, "cuTensorMapEncodeTiled failed with CUresult=%d\n", (int)res);
        return 1;
    }
    cudaMemcpy(d_tensor_map, &tensor_map, sizeof(CUtensorMap), cudaMemcpyHostToDevice);
    ...
    CUDA_SAFECALL((test_kernel<<<grid_dim, block_dim>>>(d_tensor_map, d_mat, test_type, width_stride, run_iters)));
