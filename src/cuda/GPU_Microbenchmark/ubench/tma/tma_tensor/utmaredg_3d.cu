/**
 * UTMAREDG_3D Unit Test: TMA Tensor Reduce MAX, rank-3 (Shared -> Global)
 * =========================================================================
 * Tests cp.reduce.async.bulk.tensor.3d.global.shared::cta.op::max with a
 * rank-3 CUtensorMap descriptor.
 *
 * Strategy:
 *   1. d_mat initialized to BASE_VALUE=100 for all elements
 *   2. Each thread writes pattern (1 + linear_index) into smem
 *   3. Thread (0,0,0) issues 3D TMA reduce MAX: global = max(global, shared)
 *   PASS: d_mat[idx] == max(100, 1 + idx) for in-bounds elements
 *   FAIL_NOP: elements that should increase above 100 still equal 100
 *   FAIL_MISMATCH: wrong values
 *
 * Usage: ./utmaredg_3d [-W <width>] [-H <height>] [-D <depth>] [-i <iters>]
 */
...
// Kernel
// ============================================================================
__global__ void test_UTMAREDG_3D(const __grid_constant__ CUtensorMap tensor_map,
    ...
    for (int i = 0; i < run_iters; i++) {
        if (threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0) {
            ptx::cp_reduce_async_bulk_tensor(
                ...
                ptx::op_max, &tensor_map, {x, y, z}, &smem_buffer);
...
int main(int argc, char *argv[])
{
    ...
    CUDA_SAFECALL((test_UTMAREDG_3D<<<grid, block>>>(
