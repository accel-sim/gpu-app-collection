/**
 * UTMALDG Unit Test: TMA Tensor Load (Global -> Shared)
 * ======================================================
 * Tests that cp.async.bulk.tensor correctly loads a 2D tile from global
 * memory into shared memory using a CUtensorMap descriptor.
 *
 * Strategy:
 *   1. Global source matrix initialized with unique values (1, 2, 3, ...)
 *   2. Destination buffer (d_dst) poisoned with 0xDEADBEEF
 *   3. Kernel: thread (0,0) issues TMA tensor load via mbarrier; all threads wait
 *   4. Trusted scalar copy: each thread writes smem_buffer[ty][tx] -> d_dst
 *   PASS: d_dst matches original source for in-bounds elements
 *   FAIL_NOP: d_dst still contains poison (TMA load did nothing)
 *   FAIL_MISMATCH: d_dst has wrong values (descriptor or load produced bad data)
 *
 * Usage: ./utmaldg [-w <width>] [-h <height>] [-i <iterations>]
 */
...
// Kernel
// ============================================================================
__global__ void test_UTMALDG(const __grid_constant__ CUtensorMap tensor_map,
                              int32_t *d_dst, uint32_t width_stride,
                              int run_iters)
{
...
#pragma nv_diag_suppress static_var_with_dynamic_init
    ...
    d_dst[gy * width_stride + gx] = smem_buffer[threadIdx.y][threadIdx.x];
}
...
int main(int argc, char *argv[])
{
    ...
    CUDA_SAFECALL((test_UTMALDG<<<grid, block>>>(
        tensor_map, d_dst, (uint32_t)width_stride, run_iters)));
    cudaMemcpy(h_dst, d_dst, bytes, cudaMemcpyDeviceToHost);

    printf("=== UTMALDG (TMA Tensor Load: Global -> Shared) ===\n");
