...
inline void parseGpuConfigArgs(int argc, char* argv[]) {
    ...
    auto to_uint = [](const std::string& s) { return static_cast<unsigned>(std::stoul(s)); };
    auto to_size = [](const std::string& s) { return static_cast<size_t>(std::stoull(s)); };

    for (int i = 1; i < argc - 1; ++i) {
        std::string flag(argv[i]);
        std::string val(argv[i + 1]);

        if (flag == "--sm") config.SM_NUMBER = to_uint(val);
        else if (flag == "--ws") config.WARP_SIZE = to_uint(val);
        else if (flag == "--mtsm") config.MAX_THREADS_PER_SM = to_uint(val);
        else if (flag == "--msmem") config.MAX_SHARED_MEM_SIZE = to_uint(val);
        else if (flag == "--mrpsm") config.MAX_REG_PER_SM = to_uint(val);

        else if (flag == "--mtbs") config.MAX_THREAD_BLOCK_SIZE = to_uint(val);
        else if (flag == "--msmemb") config.MAX_SHARED_MEM_SIZE_PER_BLOCK = to_uint(val);
        else if (flag == "--mrpb") config.MAX_REG_PER_BLOCK = to_uint(val);

        else if (flag == "--l1") config.L1_SIZE = to_size(val);
        else if (flag == "--l2") config.L2_SIZE = to_size(val);

        else if (flag == "--mem") config.MEM_SIZE = to_size(val);
        else if (flag == "--memclk") config.MEM_CLK_FREQUENCY = to_uint(val);
        else if (flag == "--membw") config.MEM_BITWIDTH = to_uint(val);
        else if (flag == "--clk") config.CLK_FREQUENCY = to_uint(val);

        else if (flag == "--tpb") config.THREADS_PER_BLOCK = to_uint(val);
        else if (flag == "--bpsm") config.BLOCKS_PER_SM = to_uint(val);
        else if (flag == "--tpsm") config.THREADS_PER_SM = to_uint(val);
        else if (flag == "--blocks") config.BLOCKS_NUM = to_uint(val);
        else if (flag == "--total") config.TOTAL_THREADS = to_uint(val);
